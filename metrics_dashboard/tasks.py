...
def _check_env_vars(required: list[str], optional: list[str] | None = None) -> None:
    ...
    Raises ValueError with details if any required vars are missing.
    """
    missing = [var for var in required if not os.environ.get(var)]
    if missing:
        raise ValueError(
            f"Missing required environment variables: {', '.join(missing)}. "
            f"Configure these in the Render Workflow settings."
        )

    if optional:
        for var in optional:
            if not os.environ.get(var):
                print(f"[Config] Optional env var {var} not set")
...
async def fetch_github_deployments(period_dict: dict) -> list[dict]:
    ...
    """
    try:
        _check_env_vars(["GITHUB_TOKEN", "GITHUB_ORG"])
        ...
        print(f"[GitHub Deployments] ERROR: {e}")
